...
Public Class step4
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            Using conn As SqlConnection = New SqlConnection(constr)
                Using sda As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM gmcNewSubmissionImages where submissionID = " & submissionID, conn)
    ...
    Public Shared Sub uploadYoutube(urls As Urls)
        ...
        Dim sqlText = "UPDATE gmcNewSubmission SET youtubeList = @youtubeList where id=@submissionID"
    ...
    Public Shared Sub deleteImg(urls As Urls)
        ...
        Dim sqlText = "DELETE from gmcNewSubmissionImages where Path=@imageurl"
        Using cnConnect = New SqlConnection(Common.GetConnString())
            Using cm = New SqlCommand(sqlText, cnConnect)
                cnConnect.Open()
    ...
    Protected Sub Upload(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Dim sqlText = "INSERT INTO gmcNewSubmissionImages (submissionID,Name,Path) VALUES (@submissionID,@Name, @Path)"
        Using cnConnect = New SqlConnection(Common.GetConnString())
            Using cm = New SqlCommand(sqlText, cnConnect)
                cnConnect.Open()
                cm.Parameters.AddWithValue("@submissionID", submissionID)
