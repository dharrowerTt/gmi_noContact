...
Public Class gmc_submission_index
    ...
    Protected Sub nextStep_Click(sender As Object, e As EventArgs) Handles nextStep.Click
        ...
        If submissionID < 0 Then
            ...
            Dim sqlText = "INSERT INTO gmcSubmission (orgName,location,orgSite,pocName,phoneNumber,emailAddress) VALUES (@orgName, @location, @orgSite, @pocName, @phoneNumber,@emailAddress);select id from gmcSubmission where id = @@identity"
        ...
        Else
            ...
            Dim sqlText = "UPDATE gmcSubmission SET orgName = @orgName, location = @location, orgSite= @orgSite, pocName = @pocName, phoneNumber = @phoneNumber, emailAddress= @emailAddress where id=@submissionID"
            Using cnConnect = New SqlConnection(Common.GetConnString())
                Using cm = New SqlCommand(sqlText, cnConnect)
                    cnConnect.Open()
