...
Public Class step2
    ...
    Protected Sub NextStep_Click(sender As Object, e As EventArgs) Handles NextStep.Click
        ...
        Dim sqlText = "UPDATE gmcSubmission SET activityName = @actionTitle, biogasSector = @biogasBit, coalminesSector = @coalminesBit, oilgasSector = @oilgasBit, partnerCountries = @activityLocation, partnerOrgs = @partners, activityDescription = @activityInfo, outcomes = @outcomes, nextSteps = @nextsteps where id=@submissionID"
        Using cnConnect = New SqlConnection(Common.GetConnString())
            Using cm = New SqlCommand(sqlText, cnConnect)
                ...
                cm.Parameters.Add("@activityInfo", SqlDbType.NVarChar).Value = activityinfo.Text
                cm.Parameters.Add("@outcomes", SqlDbType.NVarChar).Value = outcomes.Text
                cm.Parameters.Add("@nextsteps", SqlDbType.NVarChar).Value = nextsteps.Text
