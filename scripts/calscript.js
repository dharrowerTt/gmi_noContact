...
WTF.vars = {
	...
	grid : "grid",
	tabs : '<div class="tabs clearfix"><div id="tabCalendar" class="tab">Calendar<br>View<\/div><div id="tabList" class="tab">Month<br>Detail<\/div><div id="annualList" class="tab"><a href="event_yearlistview.aspx">Annual<br>List</a><\/div><\/div>',
	ajaxCl : 'calAjax',
	ajaxGfx : '/images/cal-ajax-loader.gif'
}
...
WTF.toggleTab = function(obj){
	if (obj.id == 'tabCalendar') {
		...
		$('#annualList').removeClass(WTF.vars.onData);
	} 
	if (obj.id == 'tabList') {
	...
	if (obj.id == 'annualList') {
		setCookie('tablist','true',1)
		$('#tabCalendar').removeClass(WTF.vars.onData);
		$('#tabList').removeClass(WTF.vars.onData);
		$('#annualList').addClass(WTF.vars.onData);
	};
	$(obj).blur();
};
...
$(document).ready(function() {
	...
	$('#tabList').click(function(){
		WTF.toggleTab(this);
		$('#annualView').hide();
		$(x).attr('class','list').attr('id','calBig').show();
	...
	$('#tabCalendar').click(function(){
		...
		$(x).attr('class','grid').attr('id','calBig').show();
		WTF.setupGrid();
		return false;
	});
